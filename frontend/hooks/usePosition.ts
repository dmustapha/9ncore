...
export function usePosition(walletAddress?: string) {
   ...
   *
   * Flow:
   *  1. Read euint128 handles from storage slots 0 (_collateral) and 1 (_debt)
  ...
  const decryptPosition = useCallback(
    async (walletClient: any) => {
      ...
      try {
        ...
        //    Contract layout: slot 0 = _collateral, slot 1 = _debt
        //    euint128 is a user-defined value type wrapping bytes32.
        ...
        // viem's signTypedData handles EIP712Domain automatically — omit it from types
        ...
        // 5. Update state with decrypted values (ClearValues keys are 0x-prefixed handles)
        const toWei = (v: any): bigint =>
          typeof v === "bigint" ? v : BigInt(String(v));

        if (rawCollateral && rawCollateral !== ZERO) {
          const val = results[rawCollateral as `0x${string}`];
          if (val !== undefined) setCollateralWei(toWei(val));
        }
        if (rawDebt && rawDebt !== ZERO) {
          const val = results[rawDebt as `0x${string}`];
          if (val !== undefined) setDebtWei(toWei(val));
        }
      } catch (e: any) {
